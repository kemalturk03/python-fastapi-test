...
async def updateStudent(id:int,request: schemas.Student,db: Session = Depends(get_db)):
    await db.query(models.Student).filter(models.Student.id == id).update(schemas.Student(firstName=request.firstName,lastName=request.lastName,number=request.number))
